...
**Pick the base first.** Use the `best_profile` field — `infra` means
`profile/resume-infra.md`, `product` means `profile/resume-product.md`. Read both
if the JD straddles them, but commit to one as the base; merging them produces a
resume that reads like neither. Say which you chose and why, in one line.
...
If the JD wants something Balaji genuinely has not done, the honest move is to
...
2. **Re-order and re-word the bullets** so the ones this JD rewards come first.
   Keep every number — the 41% / $262K AWS reduction, three weeks, 800+
   transactions/day, ~3.26M records, ~80% suspension drop, ~58% cost reduction,
   ~30% conversion lift. Numbers are the strongest thing on this resume.
